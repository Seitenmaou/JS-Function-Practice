...
function spreadTheWord(){
    window.alert(`After leaving the cat colony,
    you are never able to find it again.
    Without proof, no one believes your story,
    which passes into legend nonetheless.`)
}

function continueOn(){
    let answer = window.prompt(`You come across a chamber that extends upward to a shining light above.
    There is a long, winding staircase and a much quicker,
    but rickety-looking, ladder that lead up towards the light.
    Which do you take?`)
    if(answer === 'back'){
        goLeft()
    }else if(answer === 'ladder'){
        ladder()
    } else if(answer === 'staiicase') {
         staircase()
    }
}

function ladder(){
    window.alert(`After ascending a few feet up the ladder,
    one of its rungs snaps and you fall comedically through each of the rungs below.
    Sheepish, you return home.`)
}

function staircase(){
    window.alert(`After ascending the staircase,
    you discover a shiny blue stone,
...
function pastTheDragon(){
    let answer = window.prompt(`The dragon awakes and sits upright.
...
function draw(){
    ...
    but over time discover joy in sharing the drawing with your friends and family,
    recounting the story of your days as a sorcerer with the aide of the sketch.`)    
}
